...
def vanity_projects
  ...
  Movie.select(:id,:title, "actors.name").joins(:director).joins(:castings)
...
def most_supportive
  ...
  Actor.limit(2).group(:id).select(:id,:name,"COUNT(*) AS roles").joins(:castings)
  .where("castings.ord > 1").order("roles DESC,id ASC")
end